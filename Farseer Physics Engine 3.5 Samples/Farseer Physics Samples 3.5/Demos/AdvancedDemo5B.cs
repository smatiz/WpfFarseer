...
{
    internal class AdvancedDemo5B : PhysicsGameScreen, IDemoScreen
    {
        ...
        public override void LoadContent()
        {
            ...
            //Vector2 vertScale2 = new Vector2(ConvertUnits.ToSimUnits(1));

            //Vector2 currPos = new Vector2 { X = 100, Y = 50 }; 

            //foreach (var v in list)
            //{
            //    v.Scale(ref vertScale2);

            //    Vector2 centroid = -v.GetCentroid() + Camera.ConvertScreenToWorld(currPos);
            //    if (currPos.X < 500)
            //    {
            //        currPos += new Vector2 { X = 100, Y = 0 };
            //    }
            //    else 
            //    {
            //        currPos.X = 100;
            //        currPos += new Vector2 { X = 0, Y = 100 };
            //    }
            //    v.Translate(ref centroid);


            //    var polygonShape = new ChainShape(v);

            //    var b = new Body(World);
            //    b.CreateFixture(polygonShape);

            //}

            //return;
            //list = PolygonTools.CreatePolygon(data, alphabet.Width, 3.5f, 20, true, true);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                triangulated = new List<Vertices> 
                { 
                    new Vertices 
                    {
                        new Vector2(0,0),
